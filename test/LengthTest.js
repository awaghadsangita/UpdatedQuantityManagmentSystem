...
describe('tests for length conversion',()=>{
    ...
    it('given 0 feet object and null check for equality should return false',()=>{
        zeroFeet=new length.Length(0,length.LengthUnit.FEET);
        assert.equal(false,zeroFeet.equalality(null));
    })

    it('given 0 feet object check itself for equality should return false',()=>{
        zeroFeet=new length.Length(0,length.LengthUnit.FEET);
        assert.equal(true,zeroFeet.equalality(zeroFeet));
    })

    it('given different typeof object for equality should return false',()=>{
        zeroFeet=new length.Length(0,length.LengthUnit.FEET);
    ...
    it('one yard should be equal to one yard',()=>{
        oneYard=new length.Length(1,length.LengthUnit.YARD);
        oneYard=new length.Length(1,length.LengthUnit.YARD);
